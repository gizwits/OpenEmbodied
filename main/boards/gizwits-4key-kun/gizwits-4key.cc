...
private:
    ...
    int audio_mode_ = 0; // 0 = 正常模式 1 = 离线模式
    ...
    void InitializeButtons() {
        ...
        boot_button_.OnPressRepeat([this](uint16_t count) {
            ...
            if (count == 3) {
                if (audio_mode_ == 0) {
                    ...
                    audio_mode_ = 1;
                    Application::GetInstance().PlaySound(Lang::Sounds::P3_JI);
                } else {
                    audio_mode_ = 0;
                }
            } else if(count >= 5){
        ...
        volume_up_button_.OnPressDown([this]() {
            if (audio_mode_ == 1) {
                Application::GetInstance().ResetDecoder();
                Application::GetInstance().PlaySound(Lang::Sounds::P3_NIGANMA);
                return;
            }
        });
        ...
        volume_down_button_.OnPressDown([this]() {
            if (audio_mode_ == 1) {
                Application::GetInstance().ResetDecoder();
                Application::GetInstance().PlaySound(Lang::Sounds::P3_KUNKUN);
                return;
            }
        });
