...
private:
    ...
    uint32_t ReadBatteryAdcData() {
        ...
        // 将平均原始ADC值转换为mV（优先使用eFuse校准），再换算VBAT
        int voltage_mv = 0;
        if (adc_calibrated_) {
...
public:
    ...
    void InitializeAdc() {
        ...
        // 创建校准句柄（使用 line fitting 方案，兼容当前IDF）
        adc_cali_line_fitting_config_t cali_cfg_lf = {
            .unit_id = adc_unit_,
            .atten = ADC_ATTEN_DB_12,
            .bitwidth = ADC_BITWIDTH_12,
        };
        if (adc_cali_create_scheme_line_fitting(&cali_cfg_lf, &adc_cali_handle_) == ESP_OK) {
