...
#include "soft_uart_serial.h"  // 这个文件包含了boards/common/protocol.h并定义了mcu_protocol命名空间
...
private:
    ...
    SoftUartWrapper soft_uart_;  // 软串口实例
    SoftUartTask* soft_uart_task_ = nullptr;  // 软串口任务实例
    SoftUartSerial* soft_uart_serial_ = nullptr;  // 软串口SerialInterface实现
    Gizwits_Protocol* mcu_protocol_ = nullptr;  // MCU协议实例
...
public:
    CustomBoard() : boot_button_(BOOT_BUTTON_GPIO), audio_codec(CODEC_TX_GPIO, CODEC_RX_GPIO){      
        ...
        // 初始化软串口（GPIO4=TX, GPIO5=RX, 9600波特率）
        ESP_LOGI(TAG, "Initializing Soft UART...");
        esp_err_t uart_ret = soft_uart_.Init(MCU_SOFT_UART_TX_PIN, MCU_SOFT_UART_RX_PIN, MCU_SOFT_UART_BAUDRATE);
        if (uart_ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to initialize soft UART: %s", esp_err_to_name(uart_ret));
        } else {
            ESP_LOGI(TAG, "Soft UART initialized successfully: TX=GPIO%d, RX=GPIO%d, Baudrate=%d", 
                     MCU_SOFT_UART_TX_PIN, MCU_SOFT_UART_RX_PIN, MCU_SOFT_UART_BAUDRATE);
            
            // 创建软串口任务（专门处理软串口收发，提供更好的实时性）
            soft_uart_task_ = new SoftUartTask(&soft_uart_, (gpio_num_t)MCU_SOFT_UART_RX_PIN);
            esp_err_t task_ret = soft_uart_task_->Start();
            if (task_ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to start soft UART task: %s", esp_err_to_name(task_ret));
                delete soft_uart_task_;
                soft_uart_task_ = nullptr;
            } else {
                ESP_LOGI(TAG, "Soft UART task started successfully");
                
                // 创建SerialInterface实现（使用任务进行收发）
                soft_uart_serial_ = new SoftUartSerial(soft_uart_task_);
                
                // 配置协议参数
                ProtocolConfig protocol_config;
                protocol_config.timeout_ms = 200;
                protocol_config.retry_cnt = 3;
                
                // 创建协议实例
                mcu_protocol_ = new Gizwits_Protocol(soft_uart_serial_, protocol_config);
                
                ESP_LOGI(TAG, "MCU protocol initialized. Use QueryMCUDeviceInfo() to query device information.");
            }
        }
        ...
        // 延迟查询MCU设备信息（上电后自动发送查询）
        if (mcu_protocol_ != nullptr && soft_uart_task_ != nullptr && soft_uart_task_->IsRunning()) {
            Application::GetInstance().Schedule([this]() {
                // 延迟1秒，确保软串口任务完全启动
                vTaskDelay(pdMS_TO_TICKS(1000));
                std::vector<uint8_t> device_info;
                QueryMCUDeviceInfo(device_info);
            }, "QueryMCUDeviceInfoOnBoot");
        }
    }
    ...
    // 查询MCU设备信息（非阻塞方式，可在任何地方调用）
    bool QueryMCUDeviceInfo(std::vector<uint8_t>& device_info) {
        if (mcu_protocol_ == nullptr) {
            ESP_LOGE(TAG, "MCU protocol not initialized");
            return false;
        }
        
        ESP_LOGI(TAG, "Querying MCU device information...");
        ProtocolErrCode ret = mcu_protocol_->query_device_info(device_info);
        if (ret == ProtocolErrCode::SUCCESS) {
            ESP_LOGI(TAG, "MCU device information received, size: %zu bytes", device_info.size());
            if (device_info.size() >= 8) {
                char serial_prot_ver[9] = {0};
                memcpy(serial_prot_ver, device_info.data(), 8);
                ESP_LOGI(TAG, "Serial Protocol Version: %s", serial_prot_ver);
            }
            return true;
        } else {
            ESP_LOGE(TAG, "Failed to query MCU device information, error code: %d", static_cast<int>(ret));
            return false;
        }
    }
