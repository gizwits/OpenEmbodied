...
void WifiBoard::StartNetwork() {
    ...
    ESP_LOGI(TAG, "PRODUCT_TEST_WIFI 1");

#ifdef CONFIG_PRODUCT_TEST_WIFI
    ...
    wifi_station.OnScanResults([this](const std::vector<std::string>& ssids) {
        ...
        if (std::find(ssids.begin(), ssids.end(), CONFIG_PRODUCT_TEST_WIFI) != ssids.end()) {
            // 存在产测wifi
            Settings settings("wifi", true);
            settings.SetInt("ft_mode", 1);
