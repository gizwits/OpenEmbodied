...
// #pragma message("CONFIG_LOG_TIMESTAMP_SOURCE_RTOS3")
#define ESP_LOG_LEVEL(level, tag, format, ...) do {                     \
        if (level==ESP_LOG_ERROR )          { \
        esp_log_write(ESP_LOG_ERROR,        tag, "[%lu,%u]", esp_get_free_heap_size(),heap_caps_get_free_size(MALLOC_CAP_INTERNAL)); \
        esp_log_write(ESP_LOG_ERROR,        tag, LOG_FORMAT(E, format), esp_log_timestamp(), tag, ##__VA_ARGS__); } \
        else if (level==ESP_LOG_WARN )      { \
        esp_log_write(ESP_LOG_WARN,         tag, "[%lu,%u]", esp_get_free_heap_size(),heap_caps_get_free_size(MALLOC_CAP_INTERNAL)); \
        esp_log_write(ESP_LOG_WARN,         tag, LOG_FORMAT(W, format), esp_log_timestamp(), tag, ##__VA_ARGS__); } \
        else if (level==ESP_LOG_DEBUG )     { \
        esp_log_write(ESP_LOG_DEBUG,        tag, "[%lu,%u]", esp_get_free_heap_size(),heap_caps_get_free_size(MALLOC_CAP_INTERNAL)); \
        esp_log_write(ESP_LOG_DEBUG,        tag, LOG_FORMAT(D, format), esp_log_timestamp(), tag, ##__VA_ARGS__); } \
        else if (level==ESP_LOG_VERBOSE )   { \
        esp_log_write(ESP_LOG_VERBOSE,      tag, "[%lu,%u]", esp_get_free_heap_size(),heap_caps_get_free_size(MALLOC_CAP_INTERNAL)); \
        esp_log_write(ESP_LOG_VERBOSE,      tag, LOG_FORMAT(V, format), esp_log_timestamp(), tag, ##__VA_ARGS__); } \
        else                                { \
        esp_log_write(ESP_LOG_INFO,         tag, "[%lu,%u]", esp_get_free_heap_size(),heap_caps_get_free_size(MALLOC_CAP_INTERNAL)); \
