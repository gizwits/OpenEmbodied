...
typedef struct {
    ...
    bool is_pressed;          // 是否处于按下状态
    bool is_long_press;       // 是否为长按状态
    uint32_t press_duration;  // 按下持续时间
    uint32_t last_trigger_time; // 最后一次触发时间
...
#define TOUCH_LONG_PRESS_TIME_MS       1000
#define TOUCH_CONTINUOUS_PRESS_INTERVAL_MS 200
...
// 按键状态数组
static touch_button_state_t button_states[TOUCH_BUTTON_NUM] = {
    {
        .pad_num = TOUCH_PAD_NUM1,
        .press_start = 0,
        .is_pressed = false,
        .is_long_press = false,
        .press_duration = 0,
...
static TimerHandle_t check_all_buttons_timer = NULL;
// 定时器回调函数
static void check_all_buttons_timer_cb(TimerHandle_t xTimer) {
    // 循环检查所有按键的状态
    for(int i = 0; i < TOUCH_BUTTON_NUM; i++) {
        touch_button_state_t *btn_state = &button_states[i];
        
        // 如果按键已按下但未标记为长按
        if(btn_state->is_pressed && !btn_state->is_long_press) {
            // 计算按下时长
            btn_state->press_duration = xTaskGetTickCount() - btn_state->press_start;
            
            // 如果按下超过1秒,标记为长按并执行相应操作
            if(btn_state->press_duration >= pdMS_TO_TICKS(TOUCH_LONG_PRESS_TIME_MS)) {
                btn_state->is_long_press = true;
                btn_state->last_trigger_time = 0; // 长按触发时重置连按计时
                ESP_LOGI(TAG, "TouchSensor [%d] long press detected", btn_state->pad_num);
                
                // 长按处理逻辑
                if(btn_state->pad_num == button[0]) {
                    // 按键0长按处理
                    led_fade_cycle();
                } else if(btn_state->pad_num == button[1]) {
                    // 按键1长按处理
                    sdk_volume_cycle_control();
                }
            }
        } else if(btn_state->is_pressed && btn_state->is_long_press) {
            // 已经是长按状态,检测连按
            uint32_t current_tick = xTaskGetTickCount();
            uint32_t continuous_press_interval;
            
            // 如果last_trigger_time为0,说明是第一次触发
            if(btn_state->last_trigger_time == 0) {
                continuous_press_interval = pdMS_TO_TICKS(TOUCH_CONTINUOUS_PRESS_INTERVAL_MS); // 确保第一次会触发
            } else {
                continuous_press_interval = current_tick - btn_state->last_trigger_time;
            }
            
            // 每500ms触发一次连按
            if(continuous_press_interval >= pdMS_TO_TICKS(TOUCH_CONTINUOUS_PRESS_INTERVAL_MS)) {
                ESP_LOGI(TAG, "TouchSensor [%d] continuous press triggered", btn_state->pad_num);
                btn_state->last_trigger_time = current_tick;
...
static void tp_example_read_task(void *pvParameter)
{
    ...
    // 创建一个定时器用于检查所有按键状态
    check_all_buttons_timer = xTimerCreate("CheckButtonsTimer",
                                         pdMS_TO_TICKS(100), // 100ms定时器
                                         pdTRUE,  // 周期性触发
                                         NULL,    // 不需要传递参数
                                         check_all_buttons_timer_cb);
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "TouchSensor [%d] evt: status=0x%04X, intr_mask=0x%04X", evt.pad_num, evt.pad_status, evt.intr_mask);
        
        // 找到对应的按键状态
        touch_button_state_t *btn_state = NULL;
        for(int i = 0; i < TOUCH_BUTTON_NUM; i++) {
            if(button_states[i].pad_num == evt.pad_num) {
                btn_state = &button_states[i];
                break;
            }
        }
        
        if(btn_state == NULL) continue;
        if (evt.intr_mask & TOUCH_PAD_INTR_MASK_ACTIVE) {
            // 如果已经是长按状态，则忽略新的press事件
            if(btn_state->is_long_press) {
                continue;
            }
            
            if(btn_state->is_pressed == false) {
                btn_state->press_start = xTaskGetTickCount();
                btn_state->is_pressed = true;
                btn_state->is_long_press = false;
                btn_state->press_duration = 0;
                
                // 确保定时器创建成功后再启动
                if(check_all_buttons_timer != NULL) {
                    xTimerStart(check_all_buttons_timer, 0);
                } else {
                    ESP_LOGE(TAG, "Timer not created!");
                }
                
                ESP_LOGI(TAG, "TouchSensor [%d] pressed", btn_state->pad_num);
            }
        }
        ...
        if (evt.intr_mask & TOUCH_PAD_INTR_MASK_INACTIVE) {
            ...
            // 只有当所有按键都释放时才停止定时器
            if(all_inactive && check_all_buttons_timer != NULL) {
                xTimerStop(check_all_buttons_timer, 0);
            }
            
            // 只有当按键确实处于按下状态时才处理释放事件
            if(btn_state->is_pressed) {
                
                // 如果不是长按,则执行短按操作
                if(!btn_state->is_long_press) {
                    if(btn_state->pad_num == button[0]) {
                        ESP_LOGI(TAG, "TouchSensor [%d] short press detected, status mask 0x%"PRIu32"", btn_state->pad_num, evt.pad_status);
                        led_state_step_up();
                    } else if(btn_state->pad_num == button[1]) {
                        ESP_LOGI(TAG, "TouchSensor [%d] short press detected, status mask 0x%"PRIu32"", btn_state->pad_num, evt.pad_status);
                        sdk_volume_cycle_control();
                ...
                ESP_LOGI(TAG, "TouchSensor [%d] released, reset long press timer", btn_state->pad_num);
            }
        }
