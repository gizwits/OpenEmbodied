...
void app_main(void)
{
    ...
    while (1) {
        ...
        if (audio_event_iface_listen(evt, &msg, 1000 / portTICK_RATE_MS) != ESP_OK) {
            ...
            if (second_recorded >= 10) {
                audio_element_info_t info =  { 0 };
                audio_element_info_t i2s_info =  { 0 };
                audio_element_getinfo(music_decoder, &info);
                mp3_setinfo(music_decoder);
                audio_element_getinfo(i2s_stream_writer, &i2s_info);
                esp_err_t seek_ret = audio_element_seek(music_decoder, &seek_pos, sizeof(seek_pos), &pos, &pos_len);
                if (seek_ret == ESP_OK) {
                ...
                audio_element_set_byte_pos(fatfs_stream_reader, pos);
                audio_element_set_byte_pos(music_decoder, pos);
                audio_element_getinfo(i2s_stream_writer,&i2s_info);
