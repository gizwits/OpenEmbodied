...
#define TAG "LvlinDataPointManager"

LvlinDataPointManager& LvlinDataPointManager::GetInstance() {
    static LvlinDataPointManager instance;
    return instance;
}

// 标准实现：获取机智云协议配置
const char* LvlinDataPointManager::GetGizwitsProtocolJson() const {
    return R"json(
{
  "name": "标准设备",
...
// 标准实现：获取数据点数量
size_t LvlinDataPointManager::GetDataPointCount() const {
...
// 标准实现：生成上报数据
void LvlinDataPointManager::GenerateReportData(uint8_t* buffer, size_t buffer_size, size_t& data_size) {
...
void LvlinDataPointManager::SetCallbacks(
