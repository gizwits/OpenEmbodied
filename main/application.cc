...
// 假设摄像头分辨率为 320x240，RGB565（每像素2字节）
void CameraStatusMonitorTask(void *pvParameters)
{
...
void Application::OnLongPressCamera(bool camera_mode)
{
    auto &board = Board::GetInstance();
    auto display = board.GetDisplay();
    /* Container */
    lv_obj_t *ui_root_container = display->get_content_();

    // show_camera_feed(ui_root_container);
    if (camera_status)
    {
        show_camera_feed(ui_root_container);
        ESP_LOGI(TAG, "________________________________%d", camera_status);
    }
    else
    {
