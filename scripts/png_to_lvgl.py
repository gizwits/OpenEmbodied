...
def convert_png_to_lvgl_alpha8(png_path, output_path, var_name="spiral_img_64", resize_size=None):
    """Convert PNG to LVGL v9 lv_img_dsc_t with A8 (alpha channel only) format.
    
    - A8 format supports lv_obj_set_style_img_recolor for dynamic color changes
    - If resize_size is None, keeps original image size
    - If resize_size is provided as (width, height), resizes the image
    - Uses alpha channel from RGBA image
    """
    img = Image.open(png_path).convert('RGBA')
    
    # 如果指定了resize_size，则调整尺寸；否则保持原始尺寸
    ...
    # 根据var_name生成map变量名
    map_var_name = f"{var_name}_map"
    
    # 使用numpy优化，生成C数组
    c_bytes = ','.join(f"0x{byte:02x}" for byte in alpha.tolist())
    data_size = width * height
    
    c_code = f"""// Auto-generated by png_to_lvgl.py (A8 format)
// A8 format supports lv_obj_set_style_img_recolor for dynamic color changes
#include <lvgl.h>
static const uint8_t {map_var_name}[] = {{
    {c_bytes}
}};
...
const lv_img_dsc_t {var_name} = {{
    ...
    .data_size = sizeof({map_var_name}),
    .data = {map_var_name},
}};
"""
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(c_code)
    print(f"转换完成！输出文件：{output_path} ({width}x{height}, A8格式，支持recolor)")
